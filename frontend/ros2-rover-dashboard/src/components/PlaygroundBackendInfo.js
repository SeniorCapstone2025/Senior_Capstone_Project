'use client';

import { useState, useEffect, useRef } from 'react';
import { useRouter } from 'next/navigation';
import { motion, AnimatePresence } from 'framer-motion';
import {
  BarChart3,
  Database,
  Navigation,
  LogOut,
  RefreshCw,
  Server,
  HardDrive,
  Activity,
  Zap,
  Clock,
  Menu,
  X,
} from 'lucide-react';
import AnimatedBackground from './AnimatedBackground';
import { fadeInUpVariants, containerVariants, glowVariants, scaleInVariants } from '@/utils/animations';

const navigationItems = [
  { icon: Navigation, label: 'Dashboard', href: '/' },
  { icon: BarChart3, label: 'Inventory Metrics', href: '/inventory' },
  { icon: Database, label: 'Backend Monitor', href: '/backend', current: true },
];

export default function PlaygroundBackendInfo() {
  const [apiHealth, setApiHealth] = useState(null);
  const [dbStatus, setDbStatus] = useState(null);
  const [roverStatus, setRoverStatus] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [currentUser, setCurrentUser] = useState(null);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 });
  const [expandedSection, setExpandedSection] = useState(null);
  const router = useRouter();
  const containerRef = useRef(null);

  useEffect(() => {
    const user = localStorage.getItem('mars_user');
    const token = localStorage.getItem('mars_token');

    if (!token) {
      router.push('/login');
      return;
    }

    if (user) {
      setCurrentUser(JSON.parse(user));
    }

    fetchData();
  }, [router]);

  useEffect(() => {
    const handleMouseMove = (e) => {
      if (containerRef.current) {
        const rect = containerRef.current.getBoundingClientRect();
        const x = (e.clientX - rect.left) / rect.width;
        const y = (e.clientY - rect.top) / rect.height;
        setMousePosition({ x, y });
      }
    };

    window.addEventListener('mousemove', handleMouseMove);
    return () => window.removeEventListener('mousemove', handleMouseMove);
  }, []);

  const fetchData = async () => {
    setLoading(true);
    setError('');

    try {
      const token = localStorage.getItem('mars_token');
      const headers = { Authorization: `Bearer ${token}` };

      const [healthRes, dbRes, statusRes] = await Promise.all([
        fetch('http://localhost:8000/health', { headers }),
        fetch('http://localhost:8000/command/test-db', { headers }),
        fetch('http://localhost:8000/status/', { headers }),
      ]);

      if (healthRes.ok) setApiHealth(await healthRes.json());
      if (dbRes.ok) setDbStatus(await dbRes.json());
      if (statusRes.ok) setRoverStatus(await statusRes.json());
    } catch (err) {
      setError(err.message || 'Error fetching backend data');
    } finally {
      setLoading(false);
    }
  };

  const handleLogout = () => {
    localStorage.removeItem('mars_token');
    localStorage.removeItem('mars_user');
    router.push('/login');
  };

  const handleNavigate = (href) => {
    router.push(href);
    setMobileMenuOpen(false);
  };

  const Section = ({ title, icon: Icon, children, index }) => (
    <motion.div
      custom={index}
      variants={fadeInUpVariants}
      className="group"
    >
      <motion.button
        onClick={() => setExpandedSection(expandedSection === title ? null : title)}
        className="w-full text-left"
        whileHover={{ x: 5 }}
      >
        <div className="bg-gray-800/40 backdrop-blur-xl border border-gray-700/50 hover:border-orange-500/50 rounded-xl p-6 transition cursor-pointer">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <motion.div
                animate={glowVariants.animate}
                className="p-3 bg-gradient-to-br from-red-600 to-orange-600 rounded-lg"
              >
                <Icon className="w-6 h-6 text-white" />
              </motion.div>
              <h3 className="text-xl font-semibold text-white">{title}</h3>
            </div>

            <motion.div
              animate={{ rotate: expandedSection === title ? 180 : 0 }}
              transition={{ duration: 0.3 }}
            >
              <Zap className="w-5 h-5 text-orange-500" />
            </motion.div>
          </div>
        </div>
      </motion.button>

      <AnimatePresence>
        {expandedSection === title && (
          <motion.div
            initial={{ opacity: 0, height: 0 }}
            animate={{ opacity: 1, height: 'auto' }}
            exit={{ opacity: 0, height: 0 }}
            transition={{ duration: 0.3 }}
            className="mt-3 overflow-hidden"
          >
            <div className="bg-gray-800/20 backdrop-blur-xl border border-gray-700/30 rounded-xl p-6">
              {children}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
  );

  const StatusBadge = ({ status, label }) => (
    <div className="flex items-center justify-between mb-4 p-4 bg-gray-800/40 rounded-lg">
      <span className="text-gray-300">{label}</span>
      <motion.div
        className="w-3 h-3 rounded-full"
        animate={{
          backgroundColor:
            status === 'healthy' || status === 'active'
              ? '#10b981'
              : status === 'warning'
              ? '#f59e0b'
              : '#ef4444',
          scale: [1, 1.2, 1],
        }}
        transition={{ duration: 2, repeat: Infinity }}
      />
    </div>
  );

  return (
    <div ref={containerRef} className="min-h-screen w-full bg-gray-900 text-white overflow-hidden">
      <AnimatedBackground variant="grid" />

      {/* Header */}
      <motion.header
        className="relative z-40 border-b border-gray-800 backdrop-blur-md bg-gray-900/50"
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        transition={{ duration: 0.5 }}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <motion.div className="text-2xl font-bold bg-gradient-to-r from-red-600 to-orange-600 bg-clip-text text-transparent">
            M.A.R.S
          </motion.div>

          {/* Desktop Navigation */}
          <div className="hidden md:flex items-center gap-4">
            {navigationItems.map((item) => (
              <motion.button
                key={item.label}
                onClick={() => handleNavigate(item.href)}
                className={`px-4 py-2 rounded-lg transition flex items-center gap-2 ${
                  item.current
                    ? 'bg-orange-600/50 border border-orange-500'
                    : 'bg-gray-800/50 hover:bg-gray-800'
                }`}
                whileHover={{ scale: 1.05 }}
              >
                <item.icon className="w-5 h-5" />
                <span className="text-sm">{item.label}</span>
              </motion.button>
            ))}
          </div>

          {/* User Menu */}
          <div className="flex items-center gap-4">
            {currentUser && (
              <motion.div className="px-4 py-2 bg-gray-800/50 rounded-lg text-sm hidden sm:block">
                Welcome, <span className="font-semibold text-orange-400">{currentUser.username}</span>
              </motion.div>
            )}
            <motion.button
              onClick={handleLogout}
              className="px-4 py-2 rounded-lg bg-red-600/50 hover:bg-red-600 transition flex items-center gap-2"
              whileHover={{ scale: 1.05 }}
            >
              <LogOut className="w-5 h-5" />
              <span className="hidden sm:inline text-sm">Logout</span>
            </motion.button>

            <button
              className="md:hidden p-2"
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            >
              {mobileMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
            </button>
          </div>
        </div>

        {mobileMenuOpen && (
          <motion.div
            className="md:hidden border-t border-gray-800 bg-gray-900 p-4 space-y-2"
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
          >
            {navigationItems.map((item) => (
              <motion.button
                key={item.label}
                onClick={() => handleNavigate(item.href)}
                className="w-full px-4 py-2 rounded-lg bg-gray-800/50 hover:bg-gray-800 transition flex items-center gap-2"
              >
                <item.icon className="w-5 h-5" />
                <span>{item.label}</span>
              </motion.button>
            ))}
          </motion.div>
        )}
      </motion.header>

      {/* Main Content */}
      <motion.main
        className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8"
        variants={containerVariants}
        initial="hidden"
        animate="visible"
      >
        {/* Header */}
        <motion.div custom={0} variants={fadeInUpVariants} className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold mb-2">
              <span className="bg-gradient-to-r from-red-600 to-orange-600 bg-clip-text text-transparent">
                Backend Monitor
              </span>
            </h1>
            <p className="text-gray-400">System health, database status, and rover diagnostics</p>
          </div>

          <motion.button
            onClick={fetchData}
            disabled={loading}
            className="px-6 py-3 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 disabled:opacity-50 rounded-lg flex items-center gap-2 font-semibold"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            <RefreshCw className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} />
            Refresh
          </motion.button>
        </motion.div>

        {error && (
          <motion.div
            className="mb-6 p-4 bg-red-900/30 border border-red-500/50 rounded-lg"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          >
            <p className="text-red-300">{error}</p>
          </motion.div>
        )}

        {loading ? (
          <motion.div className="flex items-center justify-center h-64" custom={1} variants={fadeInUpVariants}>
            <motion.div
              animate={{ rotate: 360 }}
              transition={{ duration: 2, repeat: Infinity }}
              className="w-12 h-12 border-4 border-orange-600/30 border-t-orange-600 rounded-full"
            />
          </motion.div>
        ) : (
          <motion.div className="space-y-6" variants={containerVariants}>
            {/* API Health Section */}
            {apiHealth && (
              <Section title="API Health Status" icon={Server} index={1}>
                <div className="space-y-4">
                  <StatusBadge status="healthy" label="API Server Status" />
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-gray-800/40 rounded-lg">
                      <p className="text-gray-400 text-sm mb-2">Version</p>
                      <p className="text-2xl font-bold text-orange-400">{apiHealth.version || '2.0.0'}</p>
                    </div>
                    <div className="p-4 bg-gray-800/40 rounded-lg">
                      <p className="text-gray-400 text-sm mb-2">Uptime</p>
                      <p className="text-2xl font-bold text-orange-400">
                        {Math.floor((apiHealth.uptime_seconds || 0) / 60)}m
                      </p>
                    </div>
                  </div>
                  <StatusBadge
                    status={apiHealth.ros_connected ? 'active' : 'warning'}
                    label={`ROS Connection: ${apiHealth.ros_connected ? 'Connected' : 'Disconnected'}`}
                  />
                  <p className="text-sm text-gray-400 flex items-center gap-2">
                    <Clock className="w-4 h-4" />
                    Last updated: {apiHealth.timestamp}
                  </p>
                </div>
              </Section>
            )}

            {/* Database Section */}
            {dbStatus && (
              <Section title="Database Status" icon={HardDrive} index={2}>
                <div className="space-y-4">
                  <StatusBadge status="healthy" label="Database Connection" />
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-gray-800/40 rounded-lg">
                      <p className="text-gray-400 text-sm mb-2">Tables</p>
                      <p className="text-2xl font-bold text-orange-400">{dbStatus.table_count || 0}</p>
                    </div>
                    <div className="p-4 bg-gray-800/40 rounded-lg">
                      <p className="text-gray-400 text-sm mb-2">Total Records</p>
                      <p className="text-2xl font-bold text-orange-400">{dbStatus.total_rows || 0}</p>
                    </div>
                  </div>
                  <div className="p-4 bg-gray-800/40 rounded-lg">
                    <p className="text-gray-400 text-sm mb-2">Database Path</p>
                    <p className="text-sm text-orange-300 break-all font-mono">{dbStatus.db_path}</p>
                  </div>
                </div>
              </Section>
            )}

            {/* Rover Status Section */}
            {roverStatus && (
              <Section title="Rover Status" icon={Activity} index={3}>
                <div className="space-y-4">
                  <StatusBadge status="active" label={`Rover State: ${roverStatus.state}`} />
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-gray-800/40 rounded-lg">
                      <p className="text-gray-400 text-sm mb-2">Battery Level</p>
                      <div className="flex items-center gap-2">
                        <p className="text-2xl font-bold text-orange-400">{roverStatus.battery_percentage || 0}%</p>
                        <motion.div
                          className="w-16 h-2 bg-gray-700 rounded-full overflow-hidden"
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                        >
                          <motion.div
                            className="h-full bg-gradient-to-r from-orange-500 to-red-500"
                            initial={{ width: 0 }}
                            animate={{ width: `${roverStatus.battery_percentage || 0}%` }}
                            transition={{ duration: 0.5 }}
                          />
                        </motion.div>
                      </div>
                    </div>
                    <div className="p-4 bg-gray-800/40 rounded-lg">
                      <p className="text-gray-400 text-sm mb-2">FSM State</p>
                      <p className="text-lg font-bold text-orange-400">{roverStatus.fsm_state || 'N/A'}</p>
                    </div>
                  </div>
                  <div className="p-4 bg-gray-800/40 rounded-lg">
                    <p className="text-gray-400 text-sm mb-2">Current Task</p>
                    <p className="text-sm text-orange-300">{roverStatus.current_task || 'None'}</p>
                  </div>
                </div>
              </Section>
            )}
          </motion.div>
        )}
      </motion.main>

      {/* Floating Background Elements */}
      <motion.div
        className="fixed bottom-20 left-20 w-64 h-64 bg-orange-600/10 rounded-full blur-3xl pointer-events-none"
        animate={{ y: [0, 50, 0], scale: [0.8, 1.1, 0.8] }}
        transition={{ duration: 15, repeat: Infinity, delay: 2 }}
      />
    </div>
  );
}
